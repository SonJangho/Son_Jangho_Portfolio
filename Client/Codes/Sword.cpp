...
_int CSword::Update_GameObject(const _float & fTimeDelta)
{
	...
	if (0 == wcscmp(m_pUI->Get_ItemOn().szName, L"Sword"))
		m_isToolOn = true;
	else
		m_isToolOn = false;
...
HRESULT CSword::SetUp_StateDesc()
{
...
HRESULT CSword::SetUp_Resource()
{
	...
	// For.Texture_Hoe
	m_pTexture[STATE_FRONT] = (CTexture*)m_pResource_Manager->Get_Resource(CResource_Manager::TYPE_STATIC, L"Resource_Texture_SwordFront");
	if (nullptr == m_pTexture[STATE_FRONT])
		return E_FAIL;
	m_pTexture[STATE_FRONT]->AddRef();

	m_pTexture[STATE_BACK] = (CTexture*)m_pResource_Manager->Get_Resource(CResource_Manager::TYPE_STATIC, L"Resource_Texture_SwordBack");
	if (nullptr == m_pTexture[STATE_BACK])
		return E_FAIL;
	m_pTexture[STATE_BACK]->AddRef();

	m_pTexture[STATE_SIDE] = (CTexture*)m_pResource_Manager->Get_Resource(CResource_Manager::TYPE_STATIC, L"Resource_Texture_SwordSide");
...
HRESULT CSword::Reset_RenderState()
{
	if (nullptr == m_pGraphic_Device)
		return E_FAIL;
...
HRESULT CSword::Animation(_float fTimeDelta)
{
...
CGameObject * CSword::Create(LPDIRECT3DDEVICE9 pGraphic_Device)
{
