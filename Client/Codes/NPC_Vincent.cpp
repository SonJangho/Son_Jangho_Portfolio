#include "stdafx.h"
#include "..\Headers\NPC_Vincent.h"
#include "Camera.h"
#include "Management.h"

CNPC_Vincent::CNPC_Vincent(LPDIRECT3DDEVICE9 pGraphic_Device)
	: CGameObject(pGraphic_Device)
{
}

HRESULT CNPC_Vincent::Ready_GameObject()
{
	...
	Set_Position(_vec3(64*41.f, 64*26.5f, 0.f));
...
_int CNPC_Vincent::Update_GameObject(const _float& fTimeDelta)
{
...
_int CNPC_Vincent::LastUpdate_GameObject(const _float& fTimeDelta)
{
...
void CNPC_Vincent::Render_GameObject()
{
...
HRESULT CNPC_Vincent::SetUp_Resource()
{
	...
	// For.Texture
	m_pTexture = (CTexture*)m_pResource_Manager->Get_Resource(CResource_Manager::TYPE_STATIC, L"Resource_Texture_Vincent");
...
HRESULT CNPC_Vincent::SetUp_ConstantTable(LPD3DXEFFECT pEffect)
{
	if (nullptr == pEffect)
		return E_FAIL;
...
_bool CNPC_Vincent::isDraw(CGameObject * pCamera)
{
...
_matrix CNPC_Vincent::Render_TransForm(CGameObject * pCamera)
{
...
CNPC_Vincent * CNPC_Vincent::Create(LPDIRECT3DDEVICE9 pGraphic_Device)
{
	CNPC_Vincent*		pInstance = new CNPC_Vincent(pGraphic_Device);

	if (FAILED(pInstance->Ready_GameObject()))
	{
		_MSG_BOX("CNPC_Vincent Created Failed");
