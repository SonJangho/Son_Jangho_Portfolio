#include "stdafx.h"
#include "..\Headers\NPC_Robin.h"
#include "Camera.h"
#include "Management.h"

CNPC_Robin::CNPC_Robin(LPDIRECT3DDEVICE9 pGraphic_Device)
	: CGameObject(pGraphic_Device)
{
}

HRESULT CNPC_Robin::Ready_GameObject()
{
	...
	Set_Position(_vec3(64*22.5, 64*16.5, 0.f));
...
_int CNPC_Robin::Update_GameObject(const _float& fTimeDelta)
{
...
_int CNPC_Robin::LastUpdate_GameObject(const _float& fTimeDelta)
{
...
void CNPC_Robin::Render_GameObject()
{
...
HRESULT CNPC_Robin::SetUp_Resource()
{
	...
	// For.Texture
	m_pTexture = (CTexture*)m_pResource_Manager->Get_Resource(CResource_Manager::TYPE_STATIC, L"Resource_Texture_Robin");
...
HRESULT CNPC_Robin::SetUp_ConstantTable(LPD3DXEFFECT pEffect)
{
	if (nullptr == pEffect)
		return E_FAIL;
...
_bool CNPC_Robin::isDraw(CGameObject * pCamera)
{
...
_matrix CNPC_Robin::Render_TransForm(CGameObject * pCamera)
{
...
CNPC_Robin * CNPC_Robin::Create(LPDIRECT3DDEVICE9 pGraphic_Device)
{
	CNPC_Robin*		pInstance = new CNPC_Robin(pGraphic_Device);

	if (FAILED(pInstance->Ready_GameObject()))
	{
		_MSG_BOX("CNPC_Robin Created Failed");
