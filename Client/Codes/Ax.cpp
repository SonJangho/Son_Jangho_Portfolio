#include "stdafx.h"
#include "..\Headers\Ax.h"
#include "Camera.h"

CAx::CAx(LPDIRECT3DDEVICE9 pGraphic_Device)
	: CGameObject(pGraphic_Device)
{
}

HRESULT CAx::Ready_GameObject()
{
...
_int CAx::Update_GameObject(const _float & fTimeDelta)
{
	Animation(fTimeDelta);

	if (0 == wcscmp(m_pUI->Get_ItemOn().szName, L"Ax"))
		m_isToolOn = true;
	else
		m_isToolOn = false;
...
_int CAx::LastUpdate_GameObject(const _float & fTimeDelta)
{
...
HRESULT CAx::SetUp_Resource()
{
	...
	// For.Texture_Hoe
	m_pTexture[STATE_FRONT] = (CTexture*)m_pResource_Manager->Get_Resource(CResource_Manager::TYPE_STATIC, L"Resource_Texture_AxFront");
	if (nullptr == m_pTexture[STATE_FRONT])
		return E_FAIL;
	m_pTexture[STATE_FRONT]->AddRef();

	m_pTexture[STATE_BACK] = (CTexture*)m_pResource_Manager->Get_Resource(CResource_Manager::TYPE_STATIC, L"Resource_Texture_AxBack");
	if (nullptr == m_pTexture[STATE_BACK])
		return E_FAIL;
	m_pTexture[STATE_BACK]->AddRef();

	m_pTexture[STATE_SIDE] = (CTexture*)m_pResource_Manager->Get_Resource(CResource_Manager::TYPE_STATIC, L"Resource_Texture_AxSide");
...
HRESULT CAx::Animation(_float fTimeDelta)
{
...
CGameObject * CAx::Create(LPDIRECT3DDEVICE9 pGraphic_Device)
{
	CAx*		pInstance = new CAx(pGraphic_Device);

	if (FAILED(pInstance->Ready_GameObject()))
	{
		_MSG_BOX("CAx Created Failed");
