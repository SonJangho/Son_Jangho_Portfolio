#include "stdafx.h"
#include "..\Headers\WateringPot.h"
#include "Camera.h"

CWateringPot::CWateringPot(LPDIRECT3DDEVICE9 pGraphic_Device)
	: CGameObject(pGraphic_Device)
{
}

HRESULT CWateringPot::Ready_GameObject()
{
...
_int CWateringPot::Update_GameObject(const _float & fTimeDelta)
{
	Animation(fTimeDelta);

	if (0 == wcscmp(m_pUI->Get_ItemOn().szName, L"WateringPot"))
		m_isToolOn = true;
	else
		m_isToolOn = false;
...
_int CWateringPot::LastUpdate_GameObject(const _float & fTimeDelta)
{
...
void CWateringPot::Render_GameObject()
{
...
HRESULT CWateringPot::SetUp_StateDesc()
{
	...
	m_pStateDesc[STATE_BACK].iNumTexture = 3;

	m_pStateDesc[STATE_SIDE].eState = STATE_SIDE;
	m_pStateDesc[STATE_SIDE].iNumTexture = 3;

	m_pStateDesc[STATE_FRONT].eState = STATE_FRONT;
	m_pStateDesc[STATE_FRONT].iNumTexture = 3;

	return NOERROR;
}

HRESULT CWateringPot::SetUp_Resource()
{
	...
	// For.Texture_Hoe
	m_pTexture[STATE_FRONT] = (CTexture*)m_pResource_Manager->Get_Resource(CResource_Manager::TYPE_STATIC, L"Resource_Texture_WateringPotFront");
	if (nullptr == m_pTexture[STATE_FRONT])
		return E_FAIL;
	m_pTexture[STATE_FRONT]->AddRef();

	m_pTexture[STATE_BACK] = (CTexture*)m_pResource_Manager->Get_Resource(CResource_Manager::TYPE_STATIC, L"Resource_Texture_WateringPotBack");
	if (nullptr == m_pTexture[STATE_BACK])
		return E_FAIL;
	m_pTexture[STATE_BACK]->AddRef();

	m_pTexture[STATE_SIDE] = (CTexture*)m_pResource_Manager->Get_Resource(CResource_Manager::TYPE_STATIC, L"Resource_Texture_WateringPotSide");
...
HRESULT CWateringPot::SetUp_RenderState()
{
...
HRESULT CWateringPot::Reset_RenderState()
{
	if (nullptr == m_pGraphic_Device)
		return E_FAIL;
...
HRESULT CWateringPot::Animation(_float fTimeDelta)
{
...
CGameObject * CWateringPot::Create(LPDIRECT3DDEVICE9 pGraphic_Device)
{
	CWateringPot*		pInstance = new CWateringPot(pGraphic_Device);

	if (FAILED(pInstance->Ready_GameObject()))
	{
		_MSG_BOX("CWateringPot Created Failed");
