#include "stdafx.h"
#include "..\Headers\NPC_Alex.h"
#include "Camera.h"
#include "Management.h"

CNPC_Alex::CNPC_Alex(LPDIRECT3DDEVICE9 pGraphic_Device)
	: CGameObject(pGraphic_Device)
{
}

HRESULT CNPC_Alex::Ready_GameObject()
{
	...
	Set_Position(_vec3(64*17.5, 64*22.5, 0.f));
...
_int CNPC_Alex::Update_GameObject(const _float& fTimeDelta)
{
...
_int CNPC_Alex::LastUpdate_GameObject(const _float& fTimeDelta)
{
...
void CNPC_Alex::Render_GameObject()
{
...
HRESULT CNPC_Alex::SetUp_Resource()
{
	...
	// For.Texture
	m_pTexture = (CTexture*)m_pResource_Manager->Get_Resource(CResource_Manager::TYPE_STATIC, L"Resource_Texture_Alex");
...
HRESULT CNPC_Alex::SetUp_ConstantTable(LPD3DXEFFECT pEffect)
{
	if (nullptr == pEffect)
		return E_FAIL;
...
_bool CNPC_Alex::isDraw(CGameObject * pCamera)
{
...
_matrix CNPC_Alex::Render_TransForm(CGameObject * pCamera)
{
...
CNPC_Alex * CNPC_Alex::Create(LPDIRECT3DDEVICE9 pGraphic_Device)
{
	CNPC_Alex*		pInstance = new CNPC_Alex(pGraphic_Device);

	if (FAILED(pInstance->Ready_GameObject()))
	{
		_MSG_BOX("CNPC_Alex Created Failed");
