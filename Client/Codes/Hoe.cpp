#include "stdafx.h"
#include "..\Headers\Hoe.h"
#include "Camera.h"

CHoe::CHoe(LPDIRECT3DDEVICE9 pGraphic_Device)
...
_int CHoe::Update_GameObject(const _float & fTimeDelta)
{
	...
	if (0==wcscmp(m_pUI->Get_ItemOn().szName,L"Hoe"))
		m_isToolOn = true;
	else
		m_isToolOn = false;
...
_int CHoe::LastUpdate_GameObject(const _float & fTimeDelta)
{
...
HRESULT CHoe::SetUp_Resource()
{
	...
	// For.Texture_Hoe
	m_pTexture[STATE_FRONT] = (CTexture*)m_pResource_Manager->Get_Resource(CResource_Manager::TYPE_STATIC, L"Resource_Texture_HoeFront");
	if (nullptr == m_pTexture[STATE_FRONT])
		return E_FAIL;
	m_pTexture[STATE_FRONT]->AddRef();

	m_pTexture[STATE_BACK] = (CTexture*)m_pResource_Manager->Get_Resource(CResource_Manager::TYPE_STATIC, L"Resource_Texture_HoeBack");
	if (nullptr == m_pTexture[STATE_BACK])
		return E_FAIL;
	m_pTexture[STATE_BACK]->AddRef();

	m_pTexture[STATE_SIDE] = (CTexture*)m_pResource_Manager->Get_Resource(CResource_Manager::TYPE_STATIC, L"Resource_Texture_HoeSide");
...
HRESULT CHoe::SetUp_RenderState()
{
...
HRESULT CHoe::Reset_RenderState()
{
	if (nullptr == m_pGraphic_Device)
		return E_FAIL;
...
HRESULT CHoe::Animation(_float fTimeDelta)
{
...
CGameObject * CHoe::Create(LPDIRECT3DDEVICE9 pGraphic_Device)
{
	CHoe*		pInstance = new CHoe(pGraphic_Device);

	if (FAILED(pInstance->Ready_GameObject()))
	{
		_MSG_BOX("CHoe Created Failed");
