#include "stdafx.h"
#include "..\Headers\Sickle.h"
#include "Camera.h"

CSickle::CSickle(LPDIRECT3DDEVICE9 pGraphic_Device)
	: CGameObject(pGraphic_Device)
{
}

HRESULT CSickle::Ready_GameObject()
{
...
_int CSickle::Update_GameObject(const _float & fTimeDelta)
{
	Animation(fTimeDelta);

	if (0 == wcscmp(m_pUI->Get_ItemOn().szName, L"Sickle"))
		m_isToolOn = true;
	else
		m_isToolOn = false;
...
_int CSickle::LastUpdate_GameObject(const _float & fTimeDelta)
{
...
void CSickle::Render_GameObject()
{
...
HRESULT CSickle::SetUp_StateDesc()
{
	...
	m_pStateDesc[STATE_BACK].iNumTexture = 6;

	m_pStateDesc[STATE_SIDE].eState = STATE_SIDE;
	m_pStateDesc[STATE_SIDE].iNumTexture = 6;

	m_pStateDesc[STATE_FRONT].eState = STATE_FRONT;
	m_pStateDesc[STATE_FRONT].iNumTexture = 6;

	return NOERROR;
}

HRESULT CSickle::SetUp_Resource()
{
	...
	// For.Texture_Hoe
	m_pTexture[STATE_FRONT] = (CTexture*)m_pResource_Manager->Get_Resource(CResource_Manager::TYPE_STATIC, L"Resource_Texture_SickleFront");
	if (nullptr == m_pTexture[STATE_FRONT])
		return E_FAIL;
	m_pTexture[STATE_FRONT]->AddRef();

	m_pTexture[STATE_BACK] = (CTexture*)m_pResource_Manager->Get_Resource(CResource_Manager::TYPE_STATIC, L"Resource_Texture_SickleBack");
	if (nullptr == m_pTexture[STATE_BACK])
		return E_FAIL;
	m_pTexture[STATE_BACK]->AddRef();

	m_pTexture[STATE_SIDE] = (CTexture*)m_pResource_Manager->Get_Resource(CResource_Manager::TYPE_STATIC, L"Resource_Texture_SickleSide");
...
HRESULT CSickle::SetUp_RenderState()
{
...
HRESULT CSickle::Reset_RenderState()
{
	if (nullptr == m_pGraphic_Device)
		return E_FAIL;
...
HRESULT CSickle::Animation(_float fTimeDelta)
{
...
CGameObject * CSickle::Create(LPDIRECT3DDEVICE9 pGraphic_Device)
{
	CSickle*		pInstance = new CSickle(pGraphic_Device);

	if (FAILED(pInstance->Ready_GameObject()))
	{
		_MSG_BOX("CSickle Created Failed");
